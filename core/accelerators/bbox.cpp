...
void BBox::expandToInclude(const BBox& b) {
  min = glmmin3(min, b.min);
  max = glmmax3(max, b.max);
  extent = max - min;
}
...
{
    glm::vec3 t0s = (min - ray.o) * ray.inv_d;
    glm::vec3 t1s = (max - ray.o) * ray.inv_d;

    auto ts = glm::min(t0s, t1s);
    auto tl = glm::max(t0s, t1s);


    *tnear = std::max(std::max(ts.x, ts.y), ts.z);

    *tfar = std::min(std::min(tl.x, tl.y), tl.z);
